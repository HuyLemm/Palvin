...
export default function AddIncomeForm({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = () => {
    if (!title.trim())     { setError('Nhập nguồn thu.'); return; }
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(51,42,45,0.5)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, animation: 'fadeIn 0.2s ease-out' }} onClick={onClose}>
      <div style={{ background: 'var(--white)', borderRadius: 20, padding: '20px', width: '100%', maxWidth: 380, maxHeight: '80vh', overflowY: 'auto', animation: 'popIn 0.2s cubic-bezier(0.32,0.72,0,1) both' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <p style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 17, fontWeight: 700, color: 'var(--ink)' }}>Thêm khoản thu <Icon emoji="💵" size={16} /></p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <input className="input-field" placeholder="Nguồn thu là gì?" value={title} onChange={e => setTitle(e.target.value)} />
          <AmountInput placeholder="Số tiền" value={amount} onChange={setAmount} />
          <div>
            <p style={{ fontSize: 13, color: 'var(--ink-2)', marginBottom: 8, fontWeight: 500 }}>Loại</p>
          ...
          <div>
            <p style={{ fontSize: 13, color: 'var(--ink-2)', marginBottom: 8, fontWeight: 500 }}>Ai nhận</p>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button onClick={handleSubmit} style={{ flex: 2, padding: '13px', borderRadius: 14, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #5AC26A, #3D8A4E)', color: 'white', fontWeight: 700, fontSize: 15 }}>Lưu khoản thu</button>
          </div>
        </div>
      </div>
    </div>
