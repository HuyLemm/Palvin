...
const INCOME_CATEGORIES = [
  { label: 'Lương', emoji: '💵' }, { label: 'Thưởng', emoji: '🎉' },
  { label: 'Quà tặng', emoji: '🎁' }, { label: 'Đầu tư', emoji: '📈' },
  { label: 'Bán đồ', emoji: '🛍️' }, { label: 'Khác', emoji: '📦' },
];
...
export default function EditExpenseForm({ expense, onClose }: { expense: Expense; onClose: () => void }) {
  ...
  const [paidBy, setPaidBy] = useState<'Alvin' | 'Paoi' | 'Both'>(expense.paidBy);
  ...
  const handleSubmit = () => {
    if (!title.trim())     { setError(isIncome ? 'Nhập nguồn thu.' : 'Please enter a title.'); return; }
    if (!amount || isNaN(+amount)) { setError('Nhập số tiền hợp lệ.'); return; }
    setConfirmSave(true);
  };
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(51,42,45,0.5)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, animation: 'fadeIn 0.2s ease-out' }} onClick={onClose}>
      <div style={{ background: 'var(--white)', borderRadius: 20, padding: '20px', width: '100%', maxWidth: 380, maxHeight: '80vh', overflowY: 'auto', animation: 'popIn 0.2s cubic-bezier(0.32,0.72,0,1) both' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <p style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 17, fontWeight: 700, color: 'var(--ink)' }}>{isIncome ? 'Sửa khoản thu' : 'Sửa khoản chi'} <Icon emoji={isIncome ? '💵' : '💰'} size={16} /></p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <input className="input-field" placeholder={isIncome ? 'Nguồn thu là gì?' : 'What was it for?'} value={title} onChange={e => setTitle(e.target.value)} />
          <AmountInput placeholder="Số tiền" value={amount} onChange={setAmount} />
          <div>
            <p style={{ fontSize: 13, color: 'var(--ink-2)', marginBottom: 8, fontWeight: 500 }}>Danh mục</p>
          ...
          <div>
            <p style={{ fontSize: 13, color: 'var(--ink-2)', marginBottom: 8, fontWeight: 500 }}>{isIncome ? 'Ai nhận' : 'Paid by'}</p>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button onClick={handleSubmit} style={{ flex: 1, padding: '13px', borderRadius: 14, border: 'none', cursor: 'pointer', background: `linear-gradient(135deg, ${accent}, ${accentDeep})`, color: 'white', fontWeight: 700, fontSize: 15 }}>Lưu thay đổi</button>
          </div>
        </div>
      </div>
      ...
      {confirmSave && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(51,42,45,0.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, animation: 'fadeIn 0.2s ease-out' }} onClick={e => e.stopPropagation()}>
          <div style={{ background: 'var(--white)', borderRadius: 20, padding: 24, maxWidth: 280, textAlign: 'center', animation: 'popIn 0.2s cubic-bezier(0.32,0.72,0,1) both' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--ink-2)', marginBottom: 16 }}>Thông tin giao dịch sẽ được cập nhật.</p>
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={confirmSubmit} style={{ flex: 1, padding: '10px', borderRadius: 10, border: 'none', background: `linear-gradient(135deg, ${accent}, ${accentDeep})`, color: 'white', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>Lưu</button>
            </div>
          </div>
        </div>
      )}
      ...
      {confirmDelete && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(51,42,45,0.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, animation: 'fadeIn 0.2s ease-out' }} onClick={e => e.stopPropagation()}>
          <div style={{ background: 'var(--white)', borderRadius: 20, padding: 24, maxWidth: 280, textAlign: 'center', animation: 'popIn 0.2s cubic-bezier(0.32,0.72,0,1) both' }}>
            <p style={{ fontSize: 16, fontWeight: 700, marginBottom: 8, color: 'var(--ink)' }}>Xóa giao dịch này?</p>
